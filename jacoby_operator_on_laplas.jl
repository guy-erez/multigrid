...
# first column --> zeros to the left
		@inbounds y[i] = op(x[i],x[i-1],x[i+1],0.0,x[i+n1],h1,h2,w,f[i]);
	end
...
# first dot on a column --> zero above
		...
		@simd for i = (2 + colShift):(n1-1 + colShift)
			@inbounds y[i] = op(x[i],x[i-1],x[i+1],x[i-n1],x[i+n1],h1,h2,w,f[i]);
		end
		i = n1 + colShift;
# last dot on a column --> zero below
		@inbounds y[i] = op(x[i],x[i-1],0.0,x[i-n1],x[i+n1],h1,h2,w,f[i]);
	end
	colShift = n1*(n2-1);
	i = 1 + colShift;
# first dot on last column --> zeros to the right and above
	@inbounds y[i] = op(x[i],0.0,x[i+1],x[i-n1],0.0,h1,h2,w,f[i]);
	for i = (2 + colShift):(n1-1 + colShift)
# last column --> zeros to the right
		@inbounds y[i] = op(x[i],x[i-1],x[i+1],x[i-n1],0.0,h1,h2,w,f[i]);
	end
	i = n1 + colShift;
...
@time jacobi!(n,h,u,output_jacobi,w,f,200, 10^-10,multOpDirichlet!)
